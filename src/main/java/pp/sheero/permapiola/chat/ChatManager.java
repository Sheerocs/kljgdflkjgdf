...
public class ChatManager {
    ...
    public void sendSpecMessage(Player sender, String message) {
        String format = sender.hasPermission("permapiola.donor.color")
                ? this.specFormatDonator
                : this.specFormatDefault;
        ...
        for (Player p : Bukkit.getOnlinePlayers()) {
            if (DeathStateManager.isDead(p.getUniqueId())
                    || p.hasPermission("permapiola.admin")
                    || p.hasPermission("permapiola.staff")
                    || p.hasPermission("permapiola.admin.spectatorchat")) {
